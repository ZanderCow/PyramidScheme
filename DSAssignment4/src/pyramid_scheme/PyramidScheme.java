...
public class PyramidScheme extends LinkedTree<Person> {
    ...
    private ArrayList<MultiTreeNode<Person>> personChain;
     ...
     * 
     * For this one. I took the code from the findNode method and modified
     *
     ...
     * hierarchy
     */
    private ArrayList<MultiTreeNode<Person>> findNodeChain(MultiTreeNode<Person> node, Person target) {
        ArrayList<MultiTreeNode<Person>> personChain = new ArrayList();
            if (this.findNodeChain2(node, target, personChain)){ 
                return personChain;
            }
            return null;
    ...
    /**
     * Extension of findNodeChain but instead operates the recurvive calls 

     * @param node the current node being examined
     * @param target the Person being searched for
     * @param personChainn an ArrayList of people nodes
     * @return a boolean of if the target was found
     * 
     */
    private boolean findNodeChain2(MultiTreeNode<Person> node, Person target,ArrayList<MultiTreeNode<Person>> personChainn ) {
        
        if (node == null){
            personChainn = null;
            return false;
        }
        if (node.getElement().equals(target)){return true;}
        
        // for every child in 
        for (MultiTreeNode<Person> personNodeChild : node.getChildren()){ 
            if (findNodeChain2(personNodeChild,target, personChainn)){
                personChainn.add(node);
                return true;
                //when this returns true it shockwaves back up and adds 
                //the other nodes in decending order
            }
        }
        return false;
